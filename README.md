...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│ normal_rand.c   │──→ data/normal_rand*.dat
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ plot_normal_rand│──→ figures/normal_rand_hist_all.png
│     .py          │
└─────────────────┘
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│brownian_motion.c│──→ data/trajectory*.dat
└────────┬────────┘
         │
         ├─────────────────┬─────────────────┬─────────────────┐
         │                 │                 │                 │
         ▼                 ▼                 ▼                 ▼
┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
│visualize_       │ │visualize_msd.py │ │analyze_energy.py│ │analyze_diffusion│
│trajectories.py  │ │                 │ │                 │ │.py              │
└────────┬────────┘ └────────┬────────┘ └────────┬────────┘ └────────┬────────┘
         │                   │                   │                   │
         ▼                   ▼                   ▼                   ▼
...
│trajectories_2d. │ │msd_plot.png     │ │energy_distribu- │ │diffusion_param- │
│png              │ │                 │ │tion*.png         │ │eter_dependence. │
└─────────────────┘ └─────────────────┘ └─────────────────┘ └─────────────────┘
```

### ブラウン運動シミュレーションの処理フロー

```
┌─────────────────────────────────────────────────────────────┐
│ 1. パラメータ設定                                             │
│    - 温度T, 質量m, 摩擦係数γ, 時間刻みdt, ステップ数n_steps   │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────┐
...
└────────────────┬────────────────────────────────────────────┘
                 ...
                 ▼
┌─────────────────────────────────────────────────────────────┐
│ 3. 時間発展ループ (n = 0 から n_steps まで)                  │
│    ┌──────────────────────────────────────────────┐          │
│    │ 3-1. 正規分布乱数の生成 (Box-Muller変換)     │          │
│    │      η_x, η_y ~ N(0,1)                      │          │
│    └────────────────┬─────────────────────────────┘          │
│                     │                                         │
│                     ▼                                         │
...
│    │ 3-2. 速度の更新 (ランジュバン方程式)         │          │
│    │      vx = vx - (γ/m)*vx*dt +                │          │
│    │            sqrt(2γkBT/m)*sqrt(dt)*η_x        │          │
│    │      vy = vy - (γ/m)*vy*dt +                │          │
│    │            sqrt(2γkBT/m)*sqrt(dt)*η_y        │          │
│    └────────────────┬─────────────────────────────┘          │
│                     │                                         │
│                     ▼                                         │
│    ┌──────────────────────────────────────────────┐          │
...
┌─────────────────────────────────────────────────────────────┐
│ 4. データ解析                                                 │
│    - 軌道の可視化 (visualize_trajectories.py)                │
│    - MSDの計算 (visualize_msd.py)                            │
│    - エネルギー分布の解析 (analyze_energy.py)                 │
│    - 拡散係数の解析 (analyze_diffusion.py)                   │
└─────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────┐
│ 1. 一様乱数の生成                                             │
│    u1 = (rand() + 1.0) / (RAND_MAX + 2.0)  (0 < u1 < 1)    │
│    u2 = (rand() + 1.0) / (RAND_MAX + 2.0)  (0 < u2 < 1)    │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│ 1. 複数回のシミュレーション実行                               │
│    - 各実行について軌道データ (t, x, y) を取得                │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────┐
│ 2. 各試行のMSDを計算                                          │
│    MSD_i(t) = x_i(t)² + y_i(t)²  (i: 試行番号)              │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────┐
│ 3. 平均MSDを計算                                              │
│    <MSD(t)> = (1/N) * Σ MSD_i(t)  (N: 試行回数)            │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────┐
│ 4. 理論値との比較                                             │
│    理論MSD = (4kBT/γ) * [t - τ(1 - exp(-t/τ))]              │
│    ここで τ = m/γ (緩和時間)                                  │
└─────────────────────────────────────────────────────────────┘
```
...
このプロジェクトは、**ブラウン運動の数値シミュレーション**を通じて、以下の物理現象を理解することを目的としています：

1. **正規分布乱数の生成**: Box-Muller変換を用いた標準正規分布N(0,1)の乱数生成
2. **ブラウン運動のシミュレーション**: ランジュバン方程式に基づく2次元ブラウン運動の数値計算
3. **軌道の可視化**: 複数回のシミュレーション結果を重ねて表示し、ランダムな軌道を観察
4. **平均二乗変位（MSD）の解析**: 時間とともに増加する粒子の拡散を定量的に評価
5. **拡散係数の解析**: 温度、質量、摩擦係数が拡散係数に与える影響を調べ、理論値（アインシュタインの関係式: D = kBT/γ）と比較
6. **エネルギー分布の解析**: 粒子の運動エネルギー分布がボルツマン分布に従うことを確認

これらの解析により、統計物理学における重要な概念（拡散、揺動散逸定理、等分配則など）を数値的に検証することができます。
